...
if(isset($_POST['btnSimpan'])){
	...
	if (trim($cmbAmil)=="Kosong") {
		$pesanError[] = "Data <b>Amil</b> belum diisi, pilih pada combo !";		
	}
	...
	if (count($pesanError)>=1 ){
		...
		$mySql	= "INSERT INTO peminjaman (no_peminjaman, tgl_peminjaman, tgl_akan_kembali, kd_amil, keterangan, kd_petugas)
					VALUES ('$kodeBaru', '$txtTanggal', '$txtTglKembali', '$cmbAmil', '$txtKeterangan', '$userLogin')";
...
$dataAmil		= isset($_POST['cmbAmil']) ? $_POST['cmbAmil'] : '';
...
<table width="900" cellpadding="3" cellspacing="1" class="table-list">
	...
	<tr>
	  ...
	  <td width="78%"><input name="txtNomor" value="<?php echo $dataKode; ?>" size="23" maxlength="20" /></td>
	</tr>
	<tr>
      <td><strong>Tgl. Peminjaman </strong></td>
	  <td><strong>:</strong></td>
	  <td><input type="date" name="tanggal" value="<?php echo $dataTglTransaksi; ?>" /></td>
    </tr>
	<tr>
      <td><strong>Tgl. Akan Kembali </strong></td>
	  <td><strong>:</strong></td>
	  <td><input type="date" name="tglKembali" value="<?php echo $dataTglKembali; ?>" /></td>
    </tr>
	<tr>
      <td><strong>Data Amil </strong></td>
	  <td><strong>:</strong></td>
	  <td><b>
        <select name="cmbAmil">
	  ...
	  $comboSql = "SELECT * FROM amil ORDER BY kd_amil";
	  $comboQry = mysql_query($comboSql, $koneksidb) or die ("Gagal Query".mysql_error());
	  while ($comboData = mysql_fetch_array($comboQry)) {
	  	if ($dataAmil == $comboData['kd_amil']) {
			$cek = " selected";
		} else { $cek=""; }
	  	echo "<option value='$comboData[kd_amil]' $cek>[ $comboData[kd_amil] ] $comboData[nm_amil]</option>";
	  }
	  ?>
        </select>
      </b></td>
