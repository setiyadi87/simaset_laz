...
$kodeDivisi	= isset($_GET['kodeDivisi']) ? $_GET['kodeDivisi'] : 'Semua'; // dari URL
$dataDivisi	= isset($_POST['cmbDivisi']) ? $_POST['cmbDivisi'] : $kodeDivisi; // dari Form
...
if (trim($dataLokasi) =="Semua") {
	if (trim($dataDivisi) =="Semua") {
		// Jika Lokasi Kosong Semua, dan Divisi Kosong
		$filterSQL = "";
	}
	else {
		// dan Jika Lokasi Kosong (Semua), dan Divisi dipilih
		$filterSQL = " AND lokasi.kd_divisi='$dataDivisi'";
	}
}
else {
	// Jika Lokasi dipilih
...
<table width="900" border="0" cellpadding="2" cellspacing="1" class="table-border">
    ...
    <td colspan="2">
	  <table width="900" border="0"  class="table-list">
        ...
        <tr>
          <td><b> Divisi </b></td>
          <td><b>:</b></td>
          <td><select name="cmbDivisi" onChange="javascript:submitform();" >
              <option value="Semua">....</option>
              <?php
		  // Skrip menampilkan data Divisi dalam ComboBox
	  $comboSql = "SELECT * FROM divisi ORDER BY kd_divisi";
	  $comboQry = mysql_query($comboSql, $koneksidb) or die ("Gagal Query".mysql_error());
	  while ($comboData = mysql_fetch_array($comboQry)) {
		if ($comboData['kd_divisi'] == $dataDivisi) {
			$cek = " selected";
		} else { $cek=""; }
		echo "<option value='$comboData[kd_divisi]' $cek>$comboData[nm_divisi]</option>";
	  }
	  ?>
          </select></td>
        </tr>
        <tr>
          ...
          <td width="719"><select name="cmbLokasi">
              <option value="Semua"> ....</option>
              <?php
			  // Menampilkan data Lokasi per Divisi yang dipilih dari ComboBox
		  $comboSql = "SELECT * FROM lokasi WHERE kd_divisi='$dataDivisi' ORDER BY kd_lokasi";
		  $comboQry = mysql_query($comboSql, $koneksidb) or die ("Gagal Query".mysql_error());
		  while ($comboData = mysql_fetch_array($comboQry)) {
  ...
  <tr>
	...
	for ($h = 1; $h <= $max; $h++) {
		...
		echo " <a href='?page=Pencarian-Barang&hal=$list[$h]&keyWord=$dataKeyWord&kodeLokasi=$dataLokasi&kodeDivisi=$dataDivisi'>$h</a> ";
	}
	?></td>
        </tr>
    </table></td>
